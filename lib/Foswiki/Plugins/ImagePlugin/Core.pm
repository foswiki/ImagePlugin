...
sub new {
    ...
    $this->{thumbSize} = Foswiki::Func::getPreferencesValue('THUMBNAIL_SIZE')
      || 180;
    $this->{baseWeb}   = $baseWeb;
    $this->{baseTopic} = $baseTopic;
    $this->{errorMsg}  = '';           # from image mage
    $this->{autoAttachThumbnails} =
      $Foswiki::cfg{ImagePlugin}{AutoAttachThumbnails};
    $this->{autoAttachExternalImages} =
      $Foswiki::cfg{ImagePlugin}{AutoAttachExternalImages};
...
sub handleREST {
    ...
    $response->header(
        ...
        -'Content-Length' =>
          $imgInfo->{filesize},    # overrides wrong length computed by Response
...
sub handleIMAGE {
    ...
    if ( $origFile =~ /^https?:\/\/.*/ ) {
        ...
        my $dummy;
        ( $origFile, $dummy ) =
          Foswiki::Sandbox::sanitizeAttachmentName($origFile);
        ...
        unless (
            $this->mirrorImage(
                $imgWeb, $imgTopic, $url, $imgPath, $doRefresh
            )
          )
        {
        ...
        unless ( -e $imgPath ) {
            $this->{errorMsg} =
              "(1) can't find <nop>$origFile at <nop>$imgWeb.$imgTopic";
            return $this->inlineError($params);
        }
    }
    else {
        ...
        if ( $params->{topic} ) {
            ...
            ( $testWeb, $testTopic ) =
              Foswiki::Func::normalizeWebTopicName( $imgWeb, $imgTopic );
            $imgPath =
              $pubDir . '/' . $testWeb . '/' . $testTopic . '/' . $origFile;

            # you said so but it still is not there
            unless ( -e $imgPath ) {
                $this->{errorMsg} =
                  "(2) can't find <nop>$origFile at <nop>$testWeb.$testTopic";
                return $this->inlineError($params);
            }
            ...
            unless ( -e $imgPath ) {
                ...
                $albumTopic =
                  Foswiki::Func::getPreferencesValue( 'IMAGEALBUM',
                    ( $testWeb eq $theWeb ) ? undef : $testWeb );
                unless ($albumTopic) {

                    # not found, and no album
                    $this->{errorMsg} =
                      "(3) can't find <nop>$origFile in <nop>$imgWeb";
                    return $this->inlineError($params);
                }
                $albumTopic =
                  Foswiki::Func::expandCommonVariables( $albumTopic, $testTopic,
                    $testWeb );
                ( $testWeb, $testTopic ) =
                ...
                unless ( -e $imgPath ) {
                    $this->{errorMsg} =
"(4) can't find <nop>$origFile in <nop>$testWeb.$testTopic";
                    return $this->inlineError($params);
                }
...
#writeDebug("origFile=$origFile, imgWeb=$imgWeb, imgTopic=$imgTopic, imgPath=$imgPath");
    ...
    my $origFileUrl =
      $pubUrl . '/' . $imgWeb . '/' . $imgTopic . '/' . $origFile;
    $params->{alt}   ||= $origFile;
...
#writeDebug("size=$params->{size}, width=$params->{width}, height=$params->{height}");
    ...
    if ( !$result ) {
        if ( $params->{type} eq 'plain' ) {
            ...
            $result = $this->getTemplate('frame');
            my $thumbCaption =
              $this->getTemplate('magnify') . $params->{caption};
            $result =~ s/\$captionFormat/$this->getTemplate('caption')/ge;
...
# For compatibility with i18n-characters in file names, encode urls (as Foswiki.pm/viewfile does for attachment names in general)
    my $thumbFileUrl =
      $pubUrl . '/' . $imgWeb . '/' . $imgTopic . '/' . $imgInfo->{file};
    ...
    $result =~
      s/\$alt/'<noautolink>'.plainify($params->{alt}).'<\/noautolink>'/ge;
    $result =~
      s/\$title/'<noautolink>'.plainify($params->{title}).'<\/noautolink>'/ge;
    $result =~
      s/\$desc/'<noautolink>'.plainify($params->{desc}).'<\/noautolink>'/ge;
...
sub pingImage {
    ...
    unless ( defined $imgInfo->{width} ) {
        ...
        (
            ...
            $imgInfo->{filesize}, $imgInfo->{format}
        ) = $this->{mage}->Ping( $imgInfo->{imgPath} );
        $imgInfo->{width}  ||= 0;
        $imgInfo->{height} ||= 0;
    }
...
sub pingOrigImage {
    ...
    unless ( defined $imgInfo->{origWidth} ) {
        ...
        (
            ...
            $imgInfo->{origFilesize}, $imgInfo->{origFormat}
        ) = $this->{mage}->Ping( $imgInfo->{origImgPath} );
        $imgInfo->{origWidth}  ||= 0;
...
sub processImage {
    ...
    writeDebug(
"called processImage(web=$imgWeb, topic=$imgTopic, file=$imgFile, size=$size, crop=$crop, width=$width, height=$height, refresh=$doRefresh)"
    );
    ...
    my %imgInfo = (
        ...
        imgTopic    => $imgTopic,
        origFile    => $imgFile,
        origImgPath => $Foswiki::cfg{PubDir} . '/' 
          . $imgWeb . '/'
          . $imgTopic . '/'
          . $imgFile,
        file    => undef,
    ...
    if ( $size || $width || $height || $doRefresh ) {
        ...
        $doRefresh = 1
          if -f $imgInfo{imgPath}
              && getModificationTime( $imgInfo{origImgPath} ) >
              getModificationTime( $imgInfo{imgPath} );
      ...
      # is used to resize the image based on the smallest fitting dimension.
            ...
            # gravity
            if ( $crop =~
/^(on|northwest|north|northeast|west|center|east|southwest|south|southeast)$/i
              )
            {
...
# works out fine
            ...
            (
                $imgInfo{width},    $imgInfo{height},
                $imgInfo{filesize}, $imgInfo{format}
            ) = $this->{mage}->Get( 'width', 'height', 'filesize', 'format' );
            $imgInfo{width}  ||= 0;
            $imgInfo{height} ||= 0;
        }
    }
...
sub mirrorImage {
    ...
    my $response =
      $this->{ua}->get( $url, ':content_file' => $tempImgFile->filename );
    ...
    if ( $clientAborted eq 'max_size' ) {
        $this->{errorMsg} =
          "can't fetch image from <nop>'$url': max size exceeded";
        writeDebug("Error: $this->{errorMsg}");
        return 0;
    }
    ...
    $this->updateAttachment( $web, $topic, $fileName,
        { path => $url, filesize => $filesize, file => $tempImgFile } )
...
sub readImage {
    ...
    my $imgPath =
      $Foswiki::cfg{PubDir} . '/' . $web . '/' . $topic . '/' . $image;
...
sub suffixToMimeType {
    ...
    if ( $image && $image =~ /\.([^.]+)$/ ) {
        ...
        unless ( $this->{types} ) {
            $this->{types} =
              Foswiki::readFile( $Foswiki::cfg{MimeTypesFileName} );
        }
